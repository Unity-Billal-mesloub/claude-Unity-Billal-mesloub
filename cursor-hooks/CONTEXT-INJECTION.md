...
1. **Install**: `claude-Unity-Billal-mesloub cursor install` creates initial context file
2. **Stop hook**: `session-summary.sh` updates context after each session ends
3. **Cursor**: Automatically includes `cursor/rules/claude-Unity-Billal-mesloub-context.mdc` in all chats
...
```bash
claude-Unity-Billal-mesloub cursor install
```
...
cat > cursor/rules/claude-Unity-Billal-mesloub-context.mdc << EOF
---
alwaysApply: true
---
# Memory Context
...
Located at: `cursor/rules/claude-Unity-Billal-mesloub-context.mdc`

```markdown
---
alwaysApply: true
description: "Claude-Unity-Billal-mesloub context from past sessions (auto-updated)"
---

# Memory Context from Past Sessions

[Your context from claude-Unity-Billal-mesloub appears here]
...
When you run `claude-Unity-Billal-mesloub cursor install`, the project is registered in `~/claude-Unity-Billal-mesloub/cursor-projects.json`. This allows the worker to automatically update your context file whenever a new summary is generated - even if it happens from Claude Code or another IDE working on the same project.

To see registered projects:
```bash
cat ~/claude-Unity-Billal-mesloub/cursor-projects.json
```
...
| Initial setup | Automatic | `claude-Unity-Billal-mesloub cursor install` creates initial context |
...
Ask the agent: "Check claude-Unity-Billal-mesloub for any previous work on authentication"

## File Location

The context file is created at:
```
<workspace>/cursor/rules/claude-Unity-Billal-mesloub-context.mdc
```

This is version-controlled by default. Add to `.gitignore` if you don't want to commit it:
```
cursor/rules/claude-Unity-Billal-mesloub-context.mdc
```
